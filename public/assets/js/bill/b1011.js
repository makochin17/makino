...
$(function(){
    ...
    // 金額入力処理
    $('input[id^=price]').on('focus input',function () {
        // 数値しか入力させない
        var num = $(this).val();
        if (num != '') {
            num = num.replace(/[^\d.]/g, '');
    ...
    $('input[id^=unit_price]').on('change', function(e) {
        ...
        // 保管料更新
        $('input[id=price_'+listno+']').val(auto_amount_fee(unit_price, volume, rounding_code));
...
// 請求履歴ボタン押下時処理
function onBillShareSearch(url_str, list) {

    window.sessionStorage.setItem(['select_bill_code'],[]);
    var callback_id = 'callback_s1040';
...
// 配車履歴ボタン押下時処理
function onDispatchShareSearch(url_str, list) {
...
function submitChkExecution(value) {
    ...
    var flag = false;
    make_hidden('dispatch_processing_division', value, 'entryForm');
    make_hidden('processing_division', value, 'entryForm');
