...
function change() {
	...
	//庸車先ラジオボタン操作
	var clientRs = document.getElementsByName("car_radio");
	if(clientRs[1].checked) {
		document.getElementById('car_code').disabled = false;
		document.getElementById('car_search').disabled = false;
	} else {
		document.getElementById('car_code').disabled = true;
		document.getElementById('car_search').disabled = true;
	}
...
function clientSearch(url_str) {
    var callback_id = 'callback_s0050'; //IDをふる
