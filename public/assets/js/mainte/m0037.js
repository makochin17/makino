...
function change() {
	...
	if(r_value == 1) {
		...
		//document.getElementById('carrier_sales_office_code').disabled = true;
		//document.getElementById('sales_office_search').disabled = true;
	}
	if(r_value == 2){
		document.getElementById('sales_office_name').disabled = false;
		//document.getElementById('carrier_sales_office_code').disabled = false;
...
function salesOfficeSearch(url_str) {
    var callback_id = 'callback_s0032'; //IDをふる
