...
class T0080 extends \Model {
    ...
    public static function getExcelName() {
        ...
        $filename = "【".$division_name."】売上請求予定明細書（".date('Ym',  strtotime($conditions['target_date']."-01"))."）";
        return $filename;
    ...
    public static function createExcel() {
        ...
        $spreadsheet = $reader->load($tpl_dir.'template売上請求予定明細書.xlsx');
        ...
        try {
            ...
            // 操作ログ出力
            $result = OpeLog::addOpeLog('TI0020', \Config::get('m_TI0020'), '', self::$db);
            if (!$result) {
    ...
    public static function outputDetail($worksheet, $division_code, $target_date, $target_day, $client_data) {
        ...
        for ($i = $row_start; $i <= 221; $i++) {
            $worksheet->getRowDimension($i)->setVisible(false);
        }
        
        //改ページ設定
        if ($row > 42)$worksheet->setBreak("A41", Worksheet::BREAK_ROW);
        if ($row > 87)$worksheet->setBreak("A86", Worksheet::BREAK_ROW);
        if ($row > 132)$worksheet->setBreak("A131", Worksheet::BREAK_ROW);
        if ($row > 177)$worksheet->setBreak("A176", Worksheet::BREAK_ROW);
