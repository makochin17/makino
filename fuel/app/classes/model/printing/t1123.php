...
class T1123 extends \Model {
    ...
    public static function outputReport($input_list) {
        ...
        $spreadsheet = $reader->load($tpl_dir.'template納品書（三菱）.xlsx');
    ...
    public static function getExcelName($input_list) {
        ...
        $filename = "納品書（三菱マテリアル株式会社）".date('Ymd');
        return $filename;
    ...
    public static function outputHeader($worksheet, $input_data) {
        //納入先
        $worksheet->setCellValue('B6', "納入先：　".$input_data['delivery_place']);
        //納入日
        $week = array( "（日）", "（月）", "（火）", "（水）", "（木）", "（金）", "（土）" );
        $week_text = $week[date("w", strtotime($input_data['delivery_date']))];
        $worksheet->setCellValue('B10', "納入日：　".date('Y年n月j日',  strtotime($input_data['delivery_date'])).$week_text);
    }
    ...
    public static function outputDetail($spreadsheet, $sheetList, $detail_data) {
        ...
        foreach ($detail_data as $detail) {
            ...
            //品名の出力
            $worksheet->setCellValue('B'.(11 + $row), $detail['product_name']);
            //数量の出力
            $worksheet->setCellValue('E'.(11 + $row), floatval($detail['volume']));
            //備考の出力準備
            ...
            if ($row == 7) {
                //備考の出力
                $worksheet->setCellValue('A22', $remarks);
