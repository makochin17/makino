...
class M0010 extends \Model {
            ...
            if (isset($data[$head])) {
                switch ($column) {
                    case 'member_code':
                        ...
                        break;
                    case 'division_code':
                        $tmp = sprintf('%03d', $data[$head]);
                        break;
                    case 'position_code':
                        $tmp = sprintf('%02d', $data[$head]);
                        break;
                    case 'car_code':
                        $tmp = sprintf('%04d', $data[$head]);
                        break;
                    // case 'phone_number':
    ...
    public static function validData($data, $db) {
        ...
        switch (trim($data['processing_division'])) {
            ...
            case 'U':       // 更新
                ...
                // 処理区分
                if (trim($data['processing_division']) == '') {
                    return str_replace('XXXXX','項目の入力漏れ【処理区分】',\Config::get('m_MW0001'));
                } elseif (trim($data['processing_division']) != '' && strlen(trim($data['processing_division'])) != 1) {
                    return str_replace('XXXXX','桁数誤り【処理区分】',\Config::get('m_MW0001'));
                }
                // 社員コード
                ...
                if (trim($data['division_code']) == '') {
                    return str_replace('XXXXX','項目の入力漏れ【課コード】',\Config::get('m_MW0001'));
                } elseif (!preg_match('/[0-9]/', trim($data['division_code']))) {
                    return str_replace('XXXXX','データ型誤り【課コード】',\Config::get('m_MW0001'));
                } elseif (trim($data['division_code']) != '' && strlen(trim($data['division_code'])) != 3) {
                    return str_replace('XXXXX','桁数誤り【課コード】',\Config::get('m_MW0001'));
                }
                // 役職コード
                if (trim($data['position_code']) == '') {
                    return str_replace('XXXXX','項目の入力漏れ【役職コード】',\Config::get('m_MW0001'));
                } elseif (!preg_match('/[0-9]/', trim($data['position_code']))) {
                    return str_replace('XXXXX','データ型誤り【役職コード】',\Config::get('m_MW0001'));
                } elseif (trim($data['position_code']) != '' && strlen(trim($data['position_code'])) != 2) {
                    return str_replace('XXXXX','桁数誤り【役職コード】',\Config::get('m_MW0001'));
                }
                // 氏名
                ...
                if (trim($data['phone_number']) == '') {
                    return str_replace('XXXXX','項目の入力漏れ【電話番号】',\Config::get('m_MW0001'));
                } elseif (trim($data['phone_number']) != '' && strlen(trim($data['phone_number'])) > 15) {
                    return str_replace('XXXXX','桁数誤り【電話番号】',\Config::get('m_MW0001'));
                }
                // 車両コード
                if (trim($data['car_code']) != '' && !preg_match('/[0-9]/', trim($data['car_code']))) {
                    return str_replace('XXXXX','データ型誤り【車両コード】',\Config::get('m_MW0001'));
                } elseif (trim($data['car_code']) != '' && strlen(trim($data['car_code'])) != 4) {
                    return str_replace('XXXXX','桁数誤り【車両コード】',\Config::get('m_MW0001'));
                }
                // ドライバー名
                if (trim($data['driver_name']) != '' && mb_strlen(trim($data['driver_name'])) > 6) {
                    return str_replace('XXXXX','桁数誤り【ドライバー名】',\Config::get('m_MW0001'));
                }
                // ユーザー名
    ...
    public static function createTsv($db) {
        ...
        //ファイル名設定
        $title = mb_convert_encoding('社員マスタ一覧', 'SJIS', 'UTF-8');
        $fileName = $title.'.tsv';
    ...
    public static function getHeader($db) {
        ...
        $result += array("driver_name" => "ドライバー名");
        $result += array("car_code" => "車両");
        $result += array("phone_number" => "電話番号");
        $result += array("division_name" => "課");
        $result += array("position_name" => "役職");
    ...
    public static function getBody($db) {
        ...
        $stmt = \DB::select(
                ...
                array(\DB::expr('(SELECT division_name FROM m_division WHERE division_code = m.division_code)'), 'division_name'),
                array(\DB::expr('(SELECT AES_DECRYPT(UNHEX(position_name),"'.$encrypt_key.'") FROM m_position WHERE position_code = m.position_code)'), 'position_name'),
                );
    ...
    public static function addMember($data, $db = null) {
        ...
        $duplicate_key = "ON DUPLICATE KEY UPDATE `division_code` = VALUES(`division_code`)"
    ...
    public static function updMember($data, $db = null) {
        ...
        $car_code = null;
        if (!empty($data['car_code']))$car_code = $data['car_code'];
    ...
    public static function delete_record($conditions, $db = null) {
        ...
        // 操作ログ出力
        $result = OpeLog::addOpeLog('MI0007', \Config::get('m_MI0007'), '社員マスタ', $db);
        if (!$result) {
            Log::error(\Config::get('m_CE0007'));
            return \Config::get('m_CE0007');
        }
