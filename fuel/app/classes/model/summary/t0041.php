...
use \Model\Summary\T0040;

ini_set("memory_limit", "1000M");

class T0041 extends \Model {
    ...
    public static function createTsvDay() {
        $conditions = T0040::getConditions();
        $header = array('car_number' => '', 'column' => '');
        $body = array();
        ...
        //行テンプレート作成
        $body_tmp_base = array('car_number' => '', 'column' => '');
        ...
        if ($conditions['summary_category'] == 1) {
            ...
            //配車集計データ取得
            $dispatch_list = T0040::getDispatchList($conditions);
            //売上補正集計データ取得
            $sales_correction_list = T0040::getSalesCorrectionList($conditions);

            //車両リスト取得
            $car_list = T0040::createCarList($dispatch_list, $sales_correction_list);

            //車両見出し出力
            $row_num_list = array();
            $row_num = 0;
            foreach ($car_list as $car) {
                $car_number = '="'.sprintf('%04d', $car['car_number']).'"';
                $body_tmp1['car_number'] = $car_number;
                $body_tmp2['car_number'] = $car_number;
                $body_tmp3['car_number'] = $car_number;
                $body_tmp4['car_number'] = $car_number;
            ...
            foreach ($dispatch_list as $dispatch) {
                ...
                $row_num = $row_num_list[$dispatch['car_number']];
            ...
            //配車集計データ取得
            $dispatch_list = T0040::getDispatchShareList($conditions);
            //売上補正集計データ取得
            $sales_correction_list = T0040::getSalesCorrectionList($conditions);
        ...
        foreach ($sales_correction_list as $sales_correction) {
            ...
            $row_num = $row_num_list[$sales_correction['car_number']];
        ...
        //ファイル名設定
        $title = mb_convert_encoding(T0040::getExcelName(), 'SJIS', 'UTF-8');
        $fileName = $title.'.tsv';
    ...
    public static function createTsvMonth() {
        $conditions = T0040::getConditions();
        $header = array('car_number' => '', 'category' => '', 'column' => '');
        $body = array();
        ...
        //行テンプレート作成
        $body_tmp_base = array('car_number' => '', 'category' => '', 'column' => '');
        foreach ($col_num_list as $key => $val) {
            $body_tmp_base += array($val => 0);
        }
        ...
        if ($conditions['summary_category'] == 1) {
            ...
            foreach ($car_list as $car) {
                ...
                foreach ($body_tmps as $body_tmp) {
                    $body_tmp['car_number'] = '="'.sprintf('%04d', $car['car_number']).'"';
                    $body[] = $body_tmp;
        ...
        foreach ($sales_correction_list as $sales_correction) {
            ...
            $row_num = (int)$row_num_list[$sales_correction['car_number']] + (int)$shift_num;
        ...
        try {
            ...
            // 操作ログ出力
            $result = OpeLog::addOpeLog('TI0011', \Config::get('m_TI0011'), '', self::$db);
            if (!$result) {
                \Log::error(\Config::get('m_CE0007'));
            }
    ...
    public static function createTsvYear() {
        ...
        try {
            ...
            // 操作ログ出力
            $result = OpeLog::addOpeLog('TI0012', \Config::get('m_TI0012'), '', self::$db);
            if (!$result) {
                \Log::error(\Config::get('m_CE0007'));
            }
