...
class L0012 extends \Model {
     ...
     * @return int タイムスタンプ
     */
    public static function ceilDatetime($timestamp, $margin_minutes = 10){
        ...
        switch ($type) {
            ...
            default:
                $stmt = \DB::select(
                        ...
                        array('l.total_mileage', 'total_mileage'),
    ...
    public static function update_record($conditions, $db = null) {
        ...
        if ($result = self::getLogisticsById($conditions['logistics_id'], $db)) {
            if (!self::updLogistics($conditions, $db)) {
                \Log::error(\Config::get('m_DE0010')."[ID:".$conditions['logistics_id']."]");
                return \Config::get('m_DE0010');
            }
        }

        // 車両情報更新(更新失敗しても処理を止めない)
        if (self::getSearchCar($result['car_id'], $db)) {
            if (!self::updCar($result, $db)) {
                \Log::error(\Config::get('m_RE0012')."[ID:".$conditions['logistics_id']." car_code:".$conditions['car_code']." car_id:".$conditions['car_id']."]");
            }
        }

        // 操作ログ出力
        $result = OpeLog::addOpeLog('DE0005', AuthConfig::getAuthConfig('user_name').\Config::get('m_DE0005'), '出庫更新', $db);
        if (!$result) {
    ...
    public static function updLogistics($data, $db = null) {
        ...
        // 項目セット
        $set = array();
        $set['delivery_flg']        = 'YES';
        if (!empty($data['delivery_date'])) {
            $set['delivery_date']   = $data['delivery_date'];
        }
    ...
    public static function updCar($data, $db = null) {
        ...
        if ($data['tire_type'] == 'summer') {
            $set['summer_class_flg'] = 'NO';
        }
        if ($data['tire_type'] == 'winter') {
            $set['winter_class_flg'] = 'NO';
        }
