...
use \Model\Printing\T0070;
use \Model\Search\S0080;

class Controller_Printing_T0070 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '車番案内印刷';
        $cnf['page_id'] 				    = '[T0070]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '車番案内印刷';
        ...
        // 課リスト取得
        $this->division_list = GenerateList::getDivisionList(false, T0070::$db);
	}
    ...
    private function set_info(&$conditions) {
		...
		if ($code = Session::get('select_client_code')) {
            // 得意先の検索にてレコード選択された場合
            $result = S0080::getSearchClient($code, T0070::$db);
            if (count($result) > 0) {
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('output'))) {
            ...
            Session::set('t0070_list', $conditions);
