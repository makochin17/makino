...
class Controller_Bill_B1011 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        $cnf['header_title']                = '請求情報登録';
        $cnf['page_id']                     = '[B1011]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '請求情報登録';
        ...
        if ($code = Session::get('select_client_code')) {
            ...
            for($i = 0; $i < $bill_code_count; $i++){
                if ($result = B1011::getBillShare($bill_code_list[$i], B1011::$db)) {
                    ...
                    $conditions['list'][$list_no]['remarks2']                       = $result['remarks2'];
                    $conditions['list'][$list_no]['remarks3']                       = $result['remarks3'];
                    $conditions['list'][$list_no]['dispatch_number']                = '';
                    // 請求情報
        ...
        foreach ($conditions['list'] as $key => $val) {
            ...
            if (!$validation->run($value)) {
                ...
                foreach ($error as $i => $v) {
                    ...
                    } elseif ($error[$i]->rule == 'trim_max_lengths' || $error[$i]->rule == 'trim_max_lengths_int') {
        ...
        if (!empty(Input::param('input_clear')) && Input::method() == 'POST' && Security::check_token()) {
            ...
            if (empty($error_msg)) {
                ...
                foreach ($conditions['list'] as $key => $val) {
                    ...
                    // 配車番号が設定されているものが既に存在しているかチェック
                    if (!empty($val['dispatch_number'])) {
                        if ($result = B1011::checkBillShare(null, $val['dispatch_number'], B1011::$db)) {
                            $error_msg = Config::get('m_BW0004')."[配車番号　:　".sprintf('%010d', $val['dispatch_number'])."]";
                            break;
                        }
                    }
             ...
             */
            Session::delete('b1011_list');
            Session::set('b1011_list', $conditions);
        } else {
            $conditions = B1011::setForms('bill_share', $conditions, Input::param());

            if ($cond = Session::get('b1011_list', array())) {
                $conditions = $cond;
                Session::delete('b1011_list');
            }
