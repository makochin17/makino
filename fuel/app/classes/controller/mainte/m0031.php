...
class Controller_Mainte_M0031 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '保管場所列情報入力';
        $cnf['page_id'] 				    = '[M0031]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '保管場所列情報入力';
    ...
    private function validate_info() {
		...
		// 保管場所列チェック
		$validation->add('storage_column_name', '保管場所列名')
			->add_rule('required')
        ;
		$validation->run();
		return $validation;
    ...
    private function create_record($conditions) {
        ...
        $error_msg = null;
        try {
            DB::start_transaction(M0031::$db);

            $error_msg = M0031::create_record($conditions, M0031::$db);
            if (!is_null($error_msg)) {
                // トランザクションクエリをロールバックする
                DB::rollback_transaction(M0031::$db);
                return $error_msg;
            }

            DB::commit_transaction(M0031::$db);
   ...
   private function update_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0031::update_record($conditions, M0031::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
   ...
   private function delete_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0031::delete_record($conditions, M0031::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            // セッションに検索条件を設定
            Session::set('m0031_list', $conditions);
            ...
            //庸車先データ削除
            $error_msg = $this->delete_record($conditions['storage_column_id']);
         ...
         * ページング設定&検索実行
         */
        if (!$total = M0031::getSearch(true, 'all', $conditions, null, null, M0031::$db)) {
            $total = 0;
        }
        //初期表示かつ前回表示時のページ数を保持していれば、ページネーションのカレントページを設定
        $page = Session::get('m0031_page');
        ...
        Session::set('m0031_page', Input::get('p'));

        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = M0031::getSearch(false, 'all', $conditions, $offset, $limit, M0031::$db);
