...
class Controller_Mainte_M0036 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '庸車先会社情報編集';
        $cnf['page_id'] 				    = '[M0036]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '庸車先会社情報編集';
...
//            'l_carrier_company_name',
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            if ($conditions['company_radio'] == 1) {
                //変更なしの場合
                $conditions['company_name'] = $conditions['carrier_company_name'];
            } else {
                //名称変更の場合
...
//                }
            ...
            if (empty($error_msg)) {
                ...
                if (!empty($conditions['carrier_sales_office_code'])) {
                    // 次の画面へリダイレクト
                    \Response::redirect(\Uri::create('mainte/m0037'));
                } else {
                    // 庸車先情報編集画面へリダイレクト
                    \Response::redirect(\Uri::create('mainte/m0035'));
                }
