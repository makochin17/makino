...
use \Model\Mainte\M0030\M0036;
use \Model\Search\S0031;

class Controller_Mainte_M0036 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '保管場所倉庫情報入力';
        $cnf['page_id'] 				    = '[M0036]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '保管場所倉庫情報入力';
    ...
    private function validate_info() {
		...
		// 保管場所列チェック
		$validation->add('storage_warehouse_name', '保管場所列名')
			->add_rule('required')
        ;
		$validation->run();
		return $validation;
    ...
    private function create_record($conditions) {
        ...
        $error_msg = null;
        try {
            DB::start_transaction(M0036::$db);

            $error_msg = M0036::create_record($conditions, M0036::$db);
            if (!is_null($error_msg)) {
                // トランザクションクエリをロールバックする
                DB::rollback_transaction(M0036::$db);
                return $error_msg;
            }

            DB::commit_transaction(M0036::$db);
   ...
   private function update_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0036::update_record($conditions, M0036::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
   ...
   private function delete_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0036::delete_record($conditions, M0036::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            // セッションに検索条件を設定
            Session::set('m0036_list', $conditions);
            ...
            if (!empty($errors)) {
                foreach($validation->error() as $key => $e) {
                    switch ($key){
                        case 'storage_warehouse_name':
                            $error_warehouse = '保管場所倉庫名';
                            break;
                    }
                    if ($validation->error()[$key]->rule == 'required') {
                        $error_msg = str_replace('XXXXX',$error_warehouse,Config::get('m_CW0005'));
                    } elseif ($validation->error()[$key]->rule == 'valid_strings') {
                        $error_msg = str_replace('XXXXX',$error_warehouse,Config::get('m_CW0006'));
                    } elseif ($validation->error()[$key]->rule == 'is_half_katakana') {
                        $error_msg = str_replace('XXXXX',$error_warehouse,Config::get('m_CW0017'));
                    } else {
                        // $error_msg = str_replace('XXXXX',$error_warehouse,Config::get('m_CW0007'));
                    }
                    break;
                }
            }
            ...
            //庸車先データ削除
            $error_msg = $this->delete_record($conditions['storage_warehouse_id']);
         ...
         * ページング設定&検索実行
         */
        if (!$total = M0036::getSearch(true, 'all', $conditions, null, null, M0036::$db)) {
            $total = 0;
        }
        //初期表示かつ前回表示時のページ数を保持していれば、ページネーションのカレントページを設定
        $page = Session::get('m0036_page');
        ...
        Session::set('m0036_page', Input::get('p'));

        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = M0036::getSearch(false, 'all', $conditions, $offset, $limit, M0036::$db);
