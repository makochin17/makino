...
use \Model\Mainte\M0030\M0032;
use \Model\Search\S0031;

class Controller_Mainte_M0032 extends Controller_Hybrid {
    ...
    **/
    private function initViewForge($auth_data){
        ...
        $cnf['header_title']                = '保管場所奥行情報入力';
        $cnf['page_id']                     = '[M0032]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '保管場所奥行情報入力';
    ...
    private function validate_info() {
        ...
        // 保管場所奥行チェック
        $validation->add('storage_depth_name', '保管場所奥行名')
            ->add_rule('required')
        ;
        $validation->run();
        return $validation;
    ...
    private function create_record($conditions) {
        ...
        $error_msg = null;
        try {
            DB::start_transaction(M0032::$db);

            $error_msg = M0032::create_record($conditions, M0032::$db);
            if (!is_null($error_msg)) {
                // トランザクションクエリをロールバックする
                DB::rollback_transaction(M0032::$db);
                return $error_msg;
            }

            DB::commit_transaction(M0032::$db);
   ...
   private function update_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0032::update_record($conditions, M0032::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
   ...
   private function delete_record($conditions) {
       ...
       try {
           ...
           $error_msg = M0032::delete_record($conditions, M0032::$db);
           if (!is_null($error_msg)) {
               return $error_msg;
           }
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            // セッションに検索条件を設定
            Session::set('m0032_list', $conditions);
            ...
            if (!empty($errors)) {
                foreach($validation->error() as $key => $e) {
                    switch ($key){
                        case 'storage_depth_name':
                            $error_column = '保管場所奥行名';
                            break;
                    }
            ...
            //庸車先データ削除
            $error_msg = $this->delete_record($conditions['storage_depth_id']);
         ...
         * ページング設定&検索実行
         */
        if (!$total = M0032::getSearch(true, 'all', $conditions, null, null, M0032::$db)) {
            $total = 0;
        }
        //初期表示かつ前回表示時のページ数を保持していれば、ページネーションのカレントページを設定
        $page = Session::get('m0032_page');
        ...
        Session::set('m0032_page', Input::get('p'));

        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = M0032::getSearch(false, 'all', $conditions, $offset, $limit, M0032::$db);
