...
class Controller_Mainte_M0032 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '庸車先営業所情報入力';
        $cnf['page_id'] 				    = '[M0032]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '庸車先営業所情報入力';
    ...
    private function set_info(&$conditions) {
		...
		if ($code = Session::get('select_carrier_sales_office_code')) {
            ...
            $result = S0032::getSearchCarrierSalesOffice($code, null, S0032::$db);
            if (count($result) > 0) {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
            ...
            if ($conditions['sales_office_radio'] == 1) {
                ...
                if ($conditions['company_radio'] == 1) {
                    ...
                    //会社紐づきチェック
                    $result = S0032::getSearchCarrierSalesOffice($conditions['carrier_sales_office_code'], $conditions['carrier_company_code'], S0032::$db);
