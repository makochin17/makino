...
class Controller_Mainte_M0028 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '得意先部署情報入力';
        $cnf['page_id'] 				    = '[M0028]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '得意先部署情報入力';
...
//		// 得意先部署コードチェック
//		$validation->add('client_department_code', '得意先部署コード')
//			->add_rule('is_numeric');
//		$validation->run();
//		return $validation;
//	}
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            //削除の場合
            if ($conditions['department_radio'] == 3) {
            ...
            if ($conditions['department_radio'] == 1) {
                //変更なしの場合
                $conditions['department_name'] = $conditions['client_department_name'];
            } else {
                //名称変更の場合
                
                //入力値チェック
                if (empty($conditions['department_name'])) {
                    $error_msg = str_replace('XXXXX','得意先部署名',Config::get('m_CW0005'));
                }
//                //入力値チェック
//                if (empty($conditions['client_department_code'])) {
//                    $error_msg = str_replace('XXXXX','得意先部署コード',Config::get('m_CW0005'));
//                } else {
//                    //属性チェック
...
//                            // チェック項目は得意先部署コードのみのため固定
//                            $error_msg = str_replace('XXXXX','得意先部署コード',Config::get('m_CW0006'));
//                        }
//                    }
//                }
            }
