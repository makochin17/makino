...
class Controller_Mainte_M0027 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '得意先営業所情報編集';
        $cnf['page_id'] 				    = '[M0027]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '得意先営業所情報編集';
...
//            $result = S0022::getSearchClientSalesOffice($code, S0022::$db);
//            if (count($result) > 0) {
...
//            'l_client_sales_office_name',
        ...
        if (!empty(Input::param('back')) && Security::check_token()) {
            ...
            if ($conditions['sales_office_radio'] == 1) {
                //変更なしの場合
                $conditions['sales_office_name'] = $conditions['client_sales_office_name'];
            } else {
                //名称変更の場合
...
//                }
            ...
            if (empty($error_msg)) {
                ...
                if (!empty($conditions['client_department_code'])) {
                    // 次の画面へリダイレクト
                    \Response::redirect(\Uri::create('mainte/m0028'));
                } else {
                    // 得意先情報編集画面へリダイレクト
                    \Response::redirect(\Uri::create('mainte/m0025'));
                }
