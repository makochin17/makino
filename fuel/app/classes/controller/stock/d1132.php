...
class Controller_Stock_D1132 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        $cnf['header_title']                = '保管料情報編集';
        $cnf['page_id']                     = '[D1132]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '保管料情報編集';
    ...
    private function update_record($conditions) {
        ...
        // レコード存在チェック
        if (!$result = D1131::getStorageFee($conditions['storage_fee_number'], D1131::$db)) {
            return Config::get('m_DW0037');
        }

        // レコード更新
        $error_msg = D1131::update_record($conditions, D1131::$db);
    ...
    private function delete_record($conditions) {
        ...
        $error_msg = D1131::delete_record($conditions, D1131::$db);
        if (!is_null($error_msg)) {
            return $error_msg;
        }
    ...
    public function action_index() {
        ...
        $storage_fee_number                 = Input::param('storage_fee_number', '');
        ...
        if (!empty(Input::param('input_clear')) && Input::method() == 'POST' && Security::check_token()) {
                ...
                if (empty($error_msg)) {
                    ...
                    foreach ($conditions['list'] as $key => $val) {
                        ...
                        if ($val['storage_fee_code'] == '1') {
                            if ($result = D1131::checkStorageFee($storage_fee_number, $val['closing_date'], $val['client_code'], D1131::$db)) {
                ...
                try {
                    ...
                    foreach ($conditions['list'] as $key => $val) {
                        ...
                        switch ($conditions['processing_division']){
                            ...
                            case '2':
                                // 更新処理
                                $val['storage_fee_number'] = $conditions['storage_fee_number'];
             ...
             */
            Session::delete('d1132_list');
            Session::set('d1132_list', $conditions);
        } else {
            $conditions = D1131::setForms('storagefee', $conditions, Input::param());
            if ($cond = Session::get('d1132_list', array())) {
                $conditions = $cond;
                Session::delete('d1132_list');
            }

            if (!empty($storage_fee_number)) {
                Session::set('select_storage_fee_number', $storage_fee_number);
            }
