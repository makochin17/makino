...
class Controller_Stock_D1120 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        $cnf['tree']['management_function'] = '入出庫情報検索';
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UID1120", D1120::$db);
        ...
        // 課リスト取得
        $this->division_list            = GenerateList::getDivisionList(true, D1120::$db);
        // 売上ステータスリスト
        $this->sales_status_list        = GenerateList::getSalesStatusList(true, 2);
        // 入出庫区分
        $this->stock_change_list        = GenerateList::getStockChangeCategoryList(true, D1120::$db);
        // 単位リスト取得
        $this->unit_list                = GenerateList::getUnitList(true, D1120::$db);
        // 登録者リスト取得
        $this->create_user_list         = GenerateList::getCreateUserList(true, D1120::$db);
    ...
    private function set_info(&$conditions) {
		...
		if ($code = Session::get('select_client_code')) {
            // 得意先の検索にてレコード選択された場合
            $result = D1120::getSearchClient($code, D1120::$db);
            if (count($result) > 0) {
    ...
    private function update_record() {
        ...
        for ($i = 1; $i <= $upd_list_count; $i++) {
            ...
            $upd_list[] = array('stock_change_number' => Input::post('stock_change_number_'.$i, 0), 'sales_status' => Input::post('sales_status_'.$i, 1));
        }
        
        try {
            DB::start_transaction(D1120::$db);
            
            // 売上ステータス更新
            $error_msg = D1120::updateRecord($upd_list, D1120::$db);
            if (!is_null($error_msg)) {
                DB::rollback_transaction(D1120::$db);
                return $error_msg;
            }
            
            DB::commit_transaction(D1120::$db);
        ...
        echo "<script type='text/javascript'>alert('".Config::get('m_DI0032')."');</script>";
    ...
    private function delete_record() {
        ...
        $stock_change_number = Input::post('stock_change_number', '');
        
        try {
            DB::start_transaction(D1120::$db);
            
            // 入出庫レコード存在チェック
            if (!$result = D1120::getStockChange($stock_change_number, D1120::$db)) {
                return Config::get('m_DW0032');
            }
            
            // レコード削除（論理）
            $error_msg = D1120::deleteRecord($stock_change_number, $result, D1120::$db);
            if (!is_null($error_msg)) {
                DB::rollback_transaction(D1120::$db);
                return $error_msg;
            }
        ...
        echo "<script type='text/javascript'>alert('".Config::get('m_DI0033')."');</script>";
    ...
    public function action_index() {
        ...
        $conditions['stock_number'] = $stock_number;
        ...
        if ((!empty(Input::param('search')) || !empty(Input::param('search_today'))) && Security::check_token()) {
             ...
             * セッションに検索条件を設定
             */
            Session::delete('d1120_list');
            Session::set('d1120_list', $conditions);
            
        } else {
            if ($cond = Session::get('d1120_list', array())) {
         ...
         */
        if ($search_flag) {
            $total = D1120::getSearch('count', $conditions, null, null, D1120::$db, $search_mode);
            
            // 検索上限チェック
            if (Config::get('d1120_limit') < $total) {
                $error_msg = str_replace('XXXXX',Config::get('d1120_limit'),Config::get('m_DW0015'));
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = D1120::getSearch('search', $conditions, $offset, $limit, D1120::$db, $search_mode);
        ...
        //在庫データ取得
        $stock_data = D1120::getSearchStock($stock_number, D1120::$db);
