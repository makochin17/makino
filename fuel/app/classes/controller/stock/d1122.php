...
class Controller_Stock_D1122 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        $cnf['header_title']                = '入出庫情報編集';
        $cnf['page_id']                     = '[D1122]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '入出庫情報編集';
    ...
    private function set_info(&$conditions, $list_no) {
        ...
        if ($code_list = Session::get('select_stock_change_code')) {
    ...
    private function validate_info($conditions) {
        ...
        foreach ($conditions['list'] as $key => $val) {
            ...
            $validation->add('list['.$key.'][volume]', $item['volume']['name'])
                ->add_rule('required')
                ->add_rule('trim_max_lengths', $item['volume']['max_lengths'] + 6)
    ...
    private function update_record($conditions) {
        ...
        // レコード存在チェック
        if (!$result = D1121::getStockChange($conditions['stock_change_number'], D1121::$db)) {
            return Config::get('m_CW0010');
        }

        // レコード更新
        $error_msg = D1121::update_record($conditions, $result, D1121::$db);
    ...
    private function delete_record($conditions) {
        ...
        $error_msg = D1121::delete_record($conditions, $result, D1121::$db);
        if (!is_null($error_msg)) {
            return $error_msg;
        }
    ...
    public function action_index() {
        ...
        $conditions['stock_change_number'] = Input::param('stock_change_number', '');
        $select_record      = Input::param('select_record', '');
        $list_no            = Input::param('list_no', '');
        $stock_number       = Input::param('stock_number', '');
        $stock_change_number = Input::param('stock_change_number', '');
        ...
        if (!empty(Input::param('input_clear')) && Input::method() == 'POST' && Security::check_token()) {
                ...
                try {
                    ...
                    foreach ($conditions['list'] as $key => $val) {
                        ...
                        switch ($conditions['processing_division']){
                            ...
                            case '2':
                                // 更新処理
                                $val['stock_change_number'] = $conditions['stock_change_number'];
             ...
             */
            Session::delete('d1122_list');
            Session::set('d1122_list', $conditions);
        } else {
            $conditions = D1121::setForms('stock_change', $conditions, Input::param());
            if ($cond = Session::get('d1122_list', array())) {
                $conditions = $cond;
                Session::delete('d1122_list');
            }
            
            if (!empty($stock_change_number)) {
                Session::set('select_stock_change_code', $stock_change_number);
            }
