...
class Controller_Allocation_D0010 extends Controller_Hybrid {
    ...
    private function set_info(&$conditions, $list_no) {
        ...
        if ($code = Session::get('select_client_code')) {
            ...
            for($i = 0; $i < $dispatch_code_count; $i++){
                if ($result = D0010::getDispatchCharter($dispatch_code_list[$i], D0010::$db)) {
                    $list_no = $i;
...
//                            $error_column = '降地';
             ...
             */
            Session::delete('d0010_list');
            Session::set('d0010_list', $conditions);
        } else {
            $conditions = D0010::setForms('dispatch', $conditions, Input::param());
            if ($cond = Session::get('d0010_list', array())) {
                $conditions = $cond;
                Session::delete('d0010_list');
            }
