...
use \Model\Search\S1040;

class Controller_Search_S1040 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIS1040", S1040::$db);
        ...
        // 課リスト取得
        $this->division_list            = GenerateList::getDivisionList(true, S1040::$db);
        // 売上ステータスリスト
        ...
        // 地区リスト取得
        $this->area_list                = GenerateList::getAreaList(true, S1040::$db);
        // 単位リスト取得
        $this->unit_list                = GenerateList::getUnitList(true, S1040::$db);
        // 車種リスト
        $this->carmodel_list            = GenerateList::getCarModelList(true, S1040::$db);
        // 登録者リスト取得
        $this->create_user_list         = GenerateList::getCreateUserList(true, S1040::$db);
	}
    ...
    private function set_info(&$conditions) {
		...
		if ($code = Session::get('select_client_code')) {
            // 得意先の検索にてレコード選択された場合
            $result = S1040::getSearchClient($code, S1040::$db);
            if (count($result) > 0) {
            ...
            // 庸車先の検索にてレコード選択された場合
            $result = S1040::getSearchCarrier($code, S1040::$db);
            if (count($result) > 0) {
            ...
            // 車両の検索にてレコード選択された場合
            $result = S1040::getSearchCar($code, S1040::$db);
            if (count($result) > 0) {
            ...
            // 社員の検索にてレコード選択された場合
            $result = S1040::getSearchMember($code, S1040::$db);
            if (count($result) > 0) {
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
        ...
        } elseif (!empty(Input::param('select_bill_number')) && Security::check_token()) {
            // 選択ボタンが押下された場合の処理
            
            Session::set('select_bill_code', Input::param('select_bill_number'));
            Session::delete('s1040_list');
            echo "<script type='text/javascript'>window.opener[window.name]();</script>";
            echo "<script type='text/javascript'>window.close();</script>";
            // echo "<script type='text/javascript'>window.location.href='" . Uri::create('test/popuptest') ."';</script>";
            ...
            // 検索ボタンが押下された場合の処理
            $conditions = S1040::setForms($conditions, Input::param());
             ...
             */
            Session::delete('s1040_list');
            Session::set('s1040_list', $conditions);
            
        } else {
            if ($cond = Session::get('s1040_list', array())) {
         ...
         */
        if ($search_flag) {
            $total = S1040::getSearchCount($conditions, S1040::$db);
            
            // 検索上限チェック
            if (Config::get('s1040_limit') < $total) {
                $error_msg = str_replace('XXXXX',Config::get('s1040_limit'),Config::get('m_DW0015'));
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = S1040::getSearch($conditions, $offset, $limit, S1040::$db);
