...
class Controller_Search_S0021 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIS0020", S0021::$db);
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
            ...
            Session::set('select_client_company_code', Input::param('select_code'));
            Session::delete('s0021_list');
            echo "<script type='text/javascript'>window.opener[window.name]();</script>";
            echo "<script type='text/javascript'>window.close();</script>";
             ...
             */
            Session::delete('s0021_list');
            Session::set('s0021_list', $conditions);
            
        } else {
            if ($cond = Session::get('s0021_list', array())) {
         ...
         */
        if (!$init_flag) {
            $total                      = S0021::getSearch(true, $conditions, null, null, S0021::$db);
        } else {
            // 初期表示時は検索しない
            $total = 0;
        }
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = S0021::getSearch(false, $conditions, $offset, $limit, S0021::$db);
