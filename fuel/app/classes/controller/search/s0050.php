...
use \Model\Search\S0050;

class Controller_Search_S0050 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIS0050", S0050::$db);
        ...
        // 車種リスト取得
        $this->car_model_list = GenerateList::getCarModelList(true, S0050::$db);
	}
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
             ...
             */
            Session::delete('s0050_list');
            Session::set('s0050_list', $conditions);
            
        } else {
            if ($cond = Session::get('s0050_list', array())) {
         ...
         */
        if (!$init_flag) {
            $total                      = S0050::getSearch(true, $conditions, null, null, S0050::$db);
        } else {
            // 初期表示時は検索しない
            $total = 0;
        }
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = S0050::getSearch(false, $conditions, $offset, $limit, S0050::$db);
