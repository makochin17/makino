...
class Controller_Search_S0022 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIS0020", S0022::$db);
	...
	private function validate_info() {
		...
		// 得意先営業所コードチェック
		$validation->add('client_company_code', '得意先営業所コード')
			->add_rule('is_numeric');
		$validation->run();
		return $validation;
	}
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
            ...
            Session::set('select_client_sales_office_code', Input::param('select_code'));
            Session::delete('s0022_list');
            echo "<script type='text/javascript'>window.opener[window.name]();</script>";
            echo "<script type='text/javascript'>window.close();</script>";
             ...
             */
            Session::delete('s0022_list');
            Session::set('s0022_list', $conditions);
            
        } else {
            if ($cond = Session::get('s0022_list', array())) {
            ...
            if ($cond = Session::get('m0024_list', array())) {
                $conditions['client_company_code'] = $cond['client_company_code'];
            }
         ...
         */
        if (!$init_flag) {
            $total                      = S0022::getSearch(true, $conditions, null, null, S0022::$db);
        } else {
            // 初期表示時は検索しない
            $total = 0;
        }
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = S0022::getSearch(false, $conditions, $offset, $limit, S0022::$db);
