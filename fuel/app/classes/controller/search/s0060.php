...
use \Model\Search\S0060;

class Controller_Search_S0060 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIS0060", S0060::$db);
        ...
        // 分類リスト取得
        $this->category_list = GenerateList::getProductCategoryList(true);
	}
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('cancel')) && Security::check_token()) {
            ...
            Session::set('select_product_code', Input::param('select_code'));
            Session::delete('s0060_list');
            echo "<script type='text/javascript'>window.opener[window.name]();</script>";
            echo "<script type='text/javascript'>window.close();</script>";
			...
			if (!empty($errors)) {
				foreach($validation->error() as $key => $e) {
                    // チェック項目は商品コードのみのため固定
                    $error_msg = str_replace('XXXXX','商品コード',Config::get('m_CW0006'));
				}
			}
             ...
             */
            Session::delete('s0060_list');
            Session::set('s0060_list', $conditions);
            
        } else {
            if ($cond = Session::get('s0060_list', array())) {
         ...
         */
        if (!$init_flag) {
            $total                      = S0060::getSearch(true, $conditions, null, null, S0060::$db);
        } else {
            // 初期表示時は検索しない
            $total = 0;
        }
        ...
        $list_data                      = array();
        if ($total > 0) {
            $list_data                  = S0060::getSearch(false, $conditions, $offset, $limit, S0060::$db);
