...
class Controller_Summary_T0040 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '車両別売上集計';
        $cnf['page_id'] 				    = '[T0040]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '車両別売上集計';
        ...
        // 課リスト取得
        $this->division_list = GenerateList::getDivisionList(true, T0040::$db);
        // 配送区分リスト取得
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('output'))) {
            ...
            Session::set('t0040_list', $conditions);
            
            //集計開始・終了の入力チェック
            $result = T0040::checkDate();
            if (!empty($result)) {
                $error_msg = $result;
            } else {
                // 帳票出力
