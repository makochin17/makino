...
class Controller_Summary_T0030 extends Controller_Hybrid {
    ...
    **/
	private function initViewForge($auth_data){
		...
		$cnf['header_title'] 				= '庸車別売上集計';
        $cnf['page_id'] 				    = '[T0030]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '庸車別売上集計';
        ...
        // 課リスト取得
        $this->division_list = GenerateList::getDivisionList(true, T0030::$db);
        // 配送区分リスト取得
    ...
    public function action_index() {
        ...
        if (!empty(Input::param('output'))) {
            ...
            Session::set('t0030_list', $conditions);
            
            //集計開始・終了の入力チェック
            $result = T0030::checkDate();
            if (!empty($result)) {
                $error_msg = $result;
            } else {
                // 帳票出力
