...
use \Model\Logistics\L0013;

class Controller_Logistics_L0013 extends Controller_Hybrid {
    ...
    **/
    private function initViewForge($auth_data){
        ...
        $cnf['header_title']                = '出庫指示';
        $cnf['page_id']                     = '[L0013]';
        $cnf['tree']['top']                 = \Uri::base(false);
        $cnf['tree']['management_function'] = '出庫指示';
        ...
        // ページング設定値取得
        $paging_config = PagingConfig::getPagingConfig("UIL0013", L0013::$db);
        ...
        // 保管場所リスト
        $this->location_list                = GenerateList::getLocationList(true, L0013::$db);
        // ユニットリスト
        $this->unit_list                    = GenerateList::getUnitList(false, null, true, L0013::$db);
    ...
    private function set_info(&$conditions) {
        ...
        if ($code = Session::get('select_customer_code')) {
            // 得意先の検索にてレコード選択された場合
            if ($result = L0013::getSearchCustomer($code, L0013::$db)) {
            ...
            // 得意先の検索にてレコード選択された場合
            if ($result = L0013::getSearchCarByCode($code, L0013::$db)) {
            ...
            // 得意先の検索にてレコード選択された場合
            if ($result = L0013::getSearchCar($code, L0013::$db)) {
    ...
    private function update_record($conditions) {
        ...
        try {
            ...
            foreach ($ids as $logistics_id) {
                ...
                $conditions['delivery_schedule_flg']    = 'YES';
                // レコード存在チェック
                if ($result = L0013::getLogisticsById($conditions['logistics_id'], L0013::$db)) {
                    // レコード更新
                    $error_msg = L0013::update_record($conditions, L0013::$db);
            ...
            DB::commit_transaction(L0013::$db);
        ...
        echo "<script type='text/javascript'>alert('".Config::get('m_DS0013')."');</script>";
        return null;
    }
    ...
    private function delete_record($conditions) {
        ...
        try {
            ...
            if ($result = L0013::getLogisticsById($conditions['logistics_id'], L0013::$db)) {
                // レコード削除（論理）
                $error_msg = L0013::delete_record($conditions, L0013::$db);
                if (!is_null($error_msg)) {
                    DB::rollback_transaction(L0013::$db);
                    return $error_msg;
                }
        ...
        echo "<script type='text/javascript'>alert('".Config::get('m_DS0014')."');</script>";
    ...
    public function action_index() {
        ...
        $conditions         = L0013::getForms('search');

        if (!empty(Input::param('back')) && Input::method() == 'POST' && Security::check_token()) {
            // 検索画面へリダイレクト
            Session::delete('l0013_list');
            \Response::redirect(\Uri::create('logistics/l0010'));
        } elseif ($mode == 'deliveryschedulefix' && Input::method() == 'POST' && Security::check_token()) {
            // 出庫確定ボタンが押下された場合の処理
            $conditions         = L0013::setForms('logistics', $conditions, Input::param());
            $list_error_msg     = $this->update_record($conditions);
            if (!empty($list_error_msg)) {
                $list_error_msg_sub = "※出庫指示対象を選択してください。";
            }

        } elseif ($mode == 'deliveryschedulecancel' && Input::method() == 'POST' && Security::check_token()) {
            // 出庫確定ボタンが押下された場合の処理
            $conditions         = L0013::setForms('logistics', $conditions, Input::param());
            $list_error_msg     = $this->delete_record($conditions);
            if (!empty($list_error_msg)) {
                $list_error_msg_sub = "※出庫指示対象を選択してください。";
            }
             ...
             */
            Session::delete('l0013_list');
            Session::set('l0013_list', $conditions);

        } else {
            if ($cond = Session::get('l0013_list', array())) {
         ...
         */
        if ($search_flag) {
            $total = L0013::getSearch('count', $conditions, null, null, $search_mode, L0013::$db);

            // 検索上限チェック
            if (Config::get('l0013_limit') < $total) {
                $error_msg = str_replace('XXXXX',Config::get('l0013_limit'),Config::get('m_DW0015'));
        ...
        $list_data                      = array();
        if ($total > 0) {
            if ($list_data              = L0013::getSearch('search', $conditions, $offset, $limit, $search_mode, L0013::$db)) {
        ...
        $this->template->content = View::forge(AccessControl::getActiveController(),
            array(
                ...
                // 保管場所リスト(プルダウン用)
                'location_combo_list'           => array('' => '-') + L0013::getLocationList('logistics', $this->location_list, L0013::$db),
                // ユニットリスト
