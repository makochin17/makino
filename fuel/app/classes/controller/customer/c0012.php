...
class Controller_Customer_C0012 extends Controller_Hybrid {
    ...
    **/
    ...
    private function set_info(&$conditions) {
        ...
        if ($code_list = Session::get('select_customer_code')) {
            ...
            for($i = 0; $i < $customer_code_count; $i++){
                if ($result = C0011::getCustomer($customer_code_list[$i], C0011::$db)) {
                    $list_no = $i;
        ...
        } elseif (!empty($conditions['customer_code'])) {
            if ($result = C0011::getCustomer($conditions['customer_code'], C0011::$db)) {
                ...
                $conditions['customer_name_kana']     = $result['customer_name_kana'];
                $conditions['zip']                    = $result['zip'];
                $conditions['addr1']                  = $result['addr1'];
                $conditions['addr2']                  = $result['addr2'];
                $conditions['tel']                    = $result['tel'];
                $conditions['fax']                    = $result['fax'];
                $conditions['mobile']                 = $result['mobile'];
                $conditions['mail_address']           = $result['mail_address'];
                $conditions['office_name']            = $result['office_name'];
                $conditions['manager_name']           = $result['manager_name'];
                $conditions['birth_date']             = $result['birth_date'];
                $conditions['sex']                    = $result['sex'];
                $conditions['resign_flg']             = $result['resign_flg'];
                $conditions['resign_date']            = $result['resign_date'];
                $conditions['resign_reason']          = $result['resign_reason'];
            } else {
    ...
    private function update_record($conditions) {
        ...
        try {
            ...
            $error_msg = C0011::update_record($conditions, C0011::$db);
            if (!is_null($error_msg)) {
                return $error_msg;
            }
            DB::commit_transaction(C0011::$db);
        ...
        echo "<script type='text/javascript'>alert('".Config::get('m_CUS005')."');</script>";
        return null;
    }
