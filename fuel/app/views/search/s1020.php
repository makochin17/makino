<section id="banner" style="padding-top:10px;">
    <div class="content" style="margin-top:0px;">
        ...
        <?php echo Asset::js('search/s1020.js');?>
        <script>
            var processing_msg1 = '<?php echo str_replace('XXXXX','入出庫データ',Config::get('m_CW0015')); ?>';
        </script>
                ...
                <tr>
                  ...
                  <td style="width: 460px; height: 30px;">
                    <?php echo Form::select('create_user', $data['create_user'], $create_user_list,
                      array('class' => 'select-item', 'id' => 'create_user', 'style' => 'width: 180px', 'tabindex' => '12')); ?></td>
                </tr>
            </tbody>
        </table>
        ...
        <?php echo Form::hidden('select_stock_change_number', '');?>
        <?php echo Form::hidden(\Config::get('security.csrf_token_key'), \Security::fetch_token());?>
        <?php $i = 0; ?>
        <?php if ($total > 0) : ?>
                      ...
                      <?php foreach ($list_data as $key => $val) : ?>
                        ...
                        <tr>
                            <td rowspan="2" style="width: 60px; text-align: center;">
                                <?php echo Form::checkbox('select_'.$i, $val['stock_change_number'], false, array('id' => 'form_select_'.$i, 'class' => 'text', 'style' => 'display:inline;')); ?>
                                <?php echo Form::label('', 'select_'.$i, array('style' => 'display:inline;padding-left: 1.0em;', 'onclick' => 'onCheckBox('.$i.', '.$val['stock_change_number'].');')); ?>
                                &nbsp;
                                <button type="button" onclick="onSelect('<?php echo $val['stock_change_number']; ?>')" class="buttonA">選択</button>
                            </td>
