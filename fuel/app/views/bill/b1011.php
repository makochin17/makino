<main class="l-main">
  ...
  <?php echo Form::hidden('bill_number', (!empty($data['bill_number'])) ? $data['bill_number']:'', array('id' => 'hidden_bill_number'));?>
    ...
    <div class="content-row" style="margin-bottom: 20px;">
      ...
      <input type="button" name="search" value="請求履歴引用" class='buttonB' tabindex="800" onclick="onBillShareSearch('<?php echo Uri::create('search/s1040'); ?>', 0)" style="margin-left: 40px;" />
      <input type="button" name="search" value="配車履歴引用" class='buttonB' tabindex="801" onclick="onDispatchShareSearch('<?php echo Uri::create('search/s1010'); ?>', 0)" style="margin-left: 20px;" />
      <?php echo Form::submit('input_clear', '入力内容クリア', array('class' => 'buttonB', 'onclick' => 'return submitChkClear()', 'style' => 'margin-left: 20px;', 'tabindex' => '802')); ?>
    </div>
